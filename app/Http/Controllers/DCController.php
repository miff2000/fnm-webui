...
class DCController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $response = $client->request(
                'GET',
                $validatedData['api_url'] . "license",
                [
                    'auth' => [
                        $validatedData['api_username'],
                        $validatedData['api_password']
                    ]
                ]
            );
        } catch (\Exception $e) {
        ...
        if (!$res->success) {
            return back()->withErrors("API gave invalid JSON response.")
                ->withInput($request->input());
        }
        ...
        return redirect()->route('dc.show', [$new])
            ->withSuccess("DC has been created.");
    }
    ...
    public function destroy(DC $dc)
    {
        ...
        return redirect()->route('dc.index')
            ->withSuccess("DC $name has been deleted.");
    }
